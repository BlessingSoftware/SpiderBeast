...
namespace SpiderBeast.Base
{
    ...
    public class Filter
    {
        ...
        /// <returns></returns>
        public delegate FilterResult FileResultDelegate(HtmlNode node);

        public FileResultDelegate GetFilterResult;
        ...
        public virtual List<FilterResult> FiltAsRoot(HtmlNode node) {
            ...
            while (node != null)
            {
                if (rule.Verify(node))
                {
                    results.Add(GetFilterResult(node));
                }
            }
            return results;
        }
